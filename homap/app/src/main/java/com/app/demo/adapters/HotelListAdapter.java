...
public class HotelListAdapter extends BaseQuickAdapter<HotelBean, HotelListAdapter.ViewHolder> {
    int user_type;

    public HotelListAdapter(int layoutResId, @Nullable List<HotelBean> data, int user_type) {
        super(layoutResId, data);
        this.user_type = user_type;
    }

    @Override
    protected void convert(ViewHolder helper, HotelBean item) {

        helper.tv_name.setText(StringUtil.getContent(item.getName()));
        helper.tv_price.setText("￥" + item.getPrice()+ "起");
        helper.tv_star.setText("星级：" + item.getStar());
